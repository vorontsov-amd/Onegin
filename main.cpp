...
int main ()
{
    ...
    FILE* onegin_output = CloseFile ();

    fputs ("Прямая сортировка\n", onegin_output);
...
FILE* OpenFile (void)
{
    FILE* onegin_input = fopen ("input.txt", "a+");
...
struct sorting* CreateBuffer (natural MAXLENGTH_PTR, natural LENGTH_FILE)
{
    ...
    if (buffer.ptr == NULL || buffer.book == NULL)
        {
        puts ("Ошибка при создании буффера");
        exit(EXIT_FAILURE);
        }
...
FILE* CloseFile (void)
{
    FILE* onegin_output = fopen ("output.txt", "w");

    if (!onegin_output)
        {
        puts ("Ошибка при создании файла для вывода");
        exit(EXIT_FAILURE);
        }
